...
import { useDeleteCamionMutation } from "@redux/feature/services/CamionServices";
import { message, Modal } from "antd";
import React from "react";

function ModalDeleteCamion({ id, button, close }: any) {
  const [deleteCamion, { isLoading, isSuccess, isError }] =
  ...
  const Supprimer = async () => {
    try {
      await deleteCamion({ id: id }).unwrap();
      close();
    } catch (err) {
  ...
  return (
    <>
      ...
      <Modal
        title="Suppression d'un camion"
        footer={null}
        open={open}
        onCancel={handleCancel}
      >
        <>
          <AText>Voulez-vous vraiment supprimer cet camion ?</AText>
          ...
          {isSuccess &&
            message.success({
              content: "Le camion a été supprimé avec succès",
              key: 1,
            })}
          {isError &&
...
export default ModalDeleteCamion;
